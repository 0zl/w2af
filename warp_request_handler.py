...
class WarpRequestHandler:
    ...
    async def make_request(
        ...
        **kwargs,
        ...
        # 获取用于本次请求的 token (支持负载均衡)
        access_token = await self._acquire_token()
        ...
        for attempt in range(self.max_retries + 1):
            try:
                # 每次重试时都需要确保 headers 中的 token 是最新的
                # (如果 handle_token_failure 更新了 access_token)
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    ...
                    if response.status_code == 429:
                        logger.warning(
                            f"遇到 429 错误 (尝试 {attempt + 1}/{self.max_retries + 1})"
                        )

                        if attempt < self.max_retries:
                            # 尝试处理 429 并获取新 token
                            new_token = await self._handle_token_failure(access_token)
                            if new_token:
    ...
    async def _acquire_token(self) -> Optional[str]:
        """获取用于请求的 token (优先使用池 load balancing)"""
        try:
            # 尝试使用 Token 池的负载均衡
            from warp_token_pool import get_token_pool

            pool = await get_token_pool()
            # 使用 get_load_balanced_token 以支持轮询
            return await pool.get_load_balanced_token()
        except Exception as pool_error:
            # Token 池不可用，回退到 legacy 模式
            # logger.debug(f"Token 池 load balance 不可用: {pool_error}，使用回退模式")
            if not self.current_token:
                await self._refresh_token()
            return self.current_token

    async def _handle_token_failure(self, failed_token: str) -> Optional[str]:
        """处理特定 token 的失效/429"""
        try:
            from warp_token_pool import handle_token_rate_limit

            backup = await handle_token_rate_limit(failed_token)
            if backup:
                # 如果是回退模式，同时也更新 self.current_token
                if self.current_token == failed_token:
                    self.current_token = backup
                return backup
        except Exception:
            pass
    ...
    def get_stats(self) -> Dict[str, Any]:
        """获取请求处理器统计信息"""
        return {
            "current_token": self.current_token[:50] + "..."
